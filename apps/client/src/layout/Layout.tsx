...
import { isPrintMode } from '../lib/print-mode';
...
export function Layout(): React.JSX.Element {
  ...
  const printRouteMatch = useMatch({ path: '/print', end: true });
  const loginMatch = useMatch({ path: '/login', end: true });
  const printMode = isPrintMode(location.pathname, location.search);
  const brandChrome = Boolean(landingMatch || printRouteMatch || loginMatch);
  ...
  const t = landingMatch || printRouteMatch ? tLanding : tDetected;
