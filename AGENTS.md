...
- Use `:` as the segment separator in npm script names everywhere (e.g. `db:migrate`, not `db.migrate` or space-separated variants).
...
- `apps/client`: Vite 8 + React 19 + React Router v7 + shadcn/Tailwind 4; dev on port 3000, proxies `/api` → server; landing keeps the language selector hidden (translations incomplete) with en-GB as default locale; landing section order is Monorepo Features above Portfolio Demos; GitHub CTA lives on the hero outline button (no duplicate nav “View on GitHub”); landing supports `?print`/`/print` PDF export (`print-mode.ts`, `landing-print.css`, `VITE_AWS_FRONTEND_URL` for CloudFront demo links; default Save-as-PDF filename `monorepo-demo_Justin-Blair-Rankin`); login page is sign-in only (no signup footer or mode toggle). `apps/server`: Hono + @hono/node-server; `tsdown` build, `tsx watch` dev, default port 4000.
...
- `packages/core` was intentionally skipped in Phase 03; `@workspace/shared` main barrel is types/models only (server-safe); JSX components (`DemoLayout`, `StandbyPlaceholder` with optional `hint`, `OptionCard`) export via `@workspace/shared/components`; assets via `@workspace/shared/assets/*`; demo apps alias `@workspace/shared` → `packages/shared/src` (directory, required for asset subpaths). Portfolio demos: `demo-ai-pipeline` :3001 (live OpenCode LLM — responses may take ~60s to begin streaming; mock RAG grounding via prompts/fixtures, not live CKAN), `demo-datavis` :3002 (live CKAN/open data), `demo-xscan` :3003 (`demo-xscan` keeps its own monitor-icon `StandbyPlaceholder`, runs vendored `@finografic/deps-xscan` inline in an xterm terminal during dev, not QLD branding).
- `.github/workflows/deploy-aws-frontend.yml` manually deploys the canonical AWS frontend to S3 + CloudFront; `/api/*` is routed through CloudFront to the EC2-hosted Hono API backed by RDS PostgreSQL. `.github/workflows/deploy-demo-pages.yml` remains the legacy GitHub Pages workflow for static fallback only.
